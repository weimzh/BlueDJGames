...
#ifndef __DJGAME_CLIENT_BLACKJACK_H__
#define __DJGAME_CLIENT_BLACKJACK_H__
...
class BlackjackDesktop : public DJGamePokerDesktop
{
	Q_OBJECT
	public :
		BlackjackDesktop(QWidget* parent,BlackjackPanel *panel);
		~BlackjackDesktop();
	...
	protected slots :
		...
		void ClickDraw();
	private :
		BlackjackCurrent c_current;
		BlackjackPanel * c_BlackjackPanel;
		DJGameImageItem	*c_CasinoTable;
		DJGameImageItem	*c_DealerLogo;
	
		//DJGameTextItem 	*c_itemMin,*c_itemMax;
		DJMatrixCanvasText	*c_itemMin,*c_itemMax;
		DJGameTextItem	*c_itemCounterNumber[DJGAME_BLACKJACK_MAX_PLAYERS+1];
	
		quint8			c_PlayerCards[DJGAME_BLACKJACK_MAX_PLAYERS+1][BLACKJACK_PLAYER_MAX_CARDS+1];
		qint16			c_PlayerCounters[DJGAME_BLACKJACK_MAX_PLAYERS+1];
		quint8			c_PlayerDoubles[DJGAME_BLACKJACK_MAX_PLAYERS+1];
	
		quint8			c_PlayerStatus[DJGAME_BLACKJACK_MAX_PLAYERS+1];
	
		QPushButton 	*c_pbStand,*c_pbDouble,*c_pbDraw;
};


class  BlackjackPanel : public DJGamePanel
{
	Q_OBJECT
	public : 
		BlackjackPanel( DJGameController *gameController, DJGameRoom *room, const QList<quint32>& userIds, quint8 tableId, quint8 seatId, QWidget * parent = 0, Qt::WFlags f = Qt::Window);
        ~BlackjackPanel();
};
#endif
