	...
	: DJDesktopPokerController(panelController,size,parent)
{
	...
	QIcon icon	= QIcon(":/BaseRes/image/desktop/toolbutton/chu.png");
...
{
	djDebug() << "TLJDesktopController::gameTraceModel";
...
void TLJDesktopController::initUnderGameInfo( const QByteArray& buf )
{
	djDebug()<<"TLJDesktopController::initUnderGameInfo";
...
void TLJDesktopController::locateAll()
{
...
void TLJDesktopController::locateSuitPositions( QList<QPoint>& points, QList<Qt::Alignment>& aligns )
{
...
void TLJDesktopController::locateAllNonMatrimoveItems()
{
...
void TLJDesktopController::repaintAll()
{
	djDebug()<<"TLJDesktopController::repaintAll";
...
void TLJDesktopController::repaintHost()
{
...
void TLJDesktopController::fillPlayerInfo()
{
...
/*
void TLJDesktopController::repaintPlayerInfo()
{
...
*/
void TLJDesktopController::repaintShowedSuit()
{
	djDebug()<<"TLJDesktopController::repaintShowedSuit";
	djDebug()<<"m_showedPoker"<<m_showedPoker.description();
	djDebug()<<"m_numberOfShowedPokers"<<m_numberOfShowedPokers<<"m_seatOfShow"<<m_seatOfShow;
...
void TLJDesktopController::repaintShowedSuitItem( quint8 seat )
{
	djDebug()<<"TLJDesktopController::repaintShowedSuitItem"<<seat;
...
void TLJDesktopController::dealCards( int numberOfShouldHaveDealedCards )
{
	djDebug()<<"TLJDesktopController::dealCards"<<numberOfShouldHaveDealedCards;
...
void TLJDesktopController::repaintZhuang()
{
	djDebug()<<"TLJDesktopController::repaintZhuang";
...
void TLJDesktopController::repaintScore()
{
...
QByteArray TLJDesktopController::createThrowTrace( DesktopItems& items )
{	
	djDebug()<<"TLJDesktopController::createThrowTrace";
...
bool TLJDesktopController::isChuEnabled() const
{
	djDebug()<<"TLJDesktopController::isChuEnabled"<<m_throw->isEnabled()<<isWaitingThrowAcl();
	return m_throw->isEnabled() && !isWaitingThrowAcl();
}
void TLJDesktopController::checkShowStatus()
{
	djDebug()<<"TLJDesktopController::checkShowStatus";
...
quint8 TLJDesktopController::makeShowCard( int numberOfShowPokers, const DJPoker& poker )
{
	if ( 0 == numberOfShowPokers )
		return 0;
	return ((numberOfShowPokers - 1) << 6) | poker.toByte(isBridgeCard());
}
bool TLJDesktopController::isFollowing() const
{
	if ( 0 == m_masterSeat ) {
...
void TLJDesktopController::showNT()
{
	int index	= m_suit2ShowButton.value(DJPoker::Joker);
...
void TLJDesktopController::showSpade()
{
...
void TLJDesktopController::showHeart()
{
...
void TLJDesktopController::showClub()
{
...
void TLJDesktopController::showDiamond()
{
...
QList<DJPoker> TLJDesktopController::handPokersOnClass( int pokerClass, QList<DJPoker>& nonSameClassPokers )
{
...
int TLJDesktopController::findMatchingWidth( QList<DJPokerPattern>& patterns, int width )
{
...
QList<DJPokerPattern>::iterator TLJDesktopController::findMatchingPattern( QList<DJPokerPattern>& patterns,
...
DJPoker TLJDesktopController::higherSerialPoker( const DJPoker& poker, bool isStrict ) const
{
...
DJPoker TLJDesktopController::lowerSerialPoker( const DJPoker& poker, bool isStrict ) const
{
	DJPoker::Point point	= poker.point();
	int	suit	= poker.suit();
	int index	= m_series.indexOf( point );
...
void TLJDesktopController::clickTip()
{	
	if ( !isFollowing() )
		return;
...
void TLJDesktopController::clickThrow()
{
	throwSelectedCards();
}
void TLJDesktopController::previousPressed()
{
	for ( int seat = 1; seat <= panelController()->numberOfSeats(); seat++ ) {
...
void TLJDesktopController::previousRelesed()
{
	for ( int seat = 1; seat <= panelController()->numberOfSeats(); seat++ ) {
		hideDesktopItems( seat, POKER_TYPE_LASTROUND_THROWN );
...
void TLJDesktopController::subtractPressed()
{
...
void TLJDesktopController::subtractReleased()
{
	hideDesktopItems( m_current.chMaster, POKER_TYPE_SUBTRACT );
	showDesktopItems( m_current.chMaster, POKER_TYPE_HAND );
}
QString	TLJDesktopController::userItemNameSuffix( DJGameUser* user ) const
{
	djDebug()<<"TLJDesktopController::userItemNameSuffix"<<user->userName()<<user->seatId()<<m_current.chMaster;
	if ( 0 != m_current.chMaster 
