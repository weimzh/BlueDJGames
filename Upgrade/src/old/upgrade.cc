...
UpgradeDesktop::UpgradeDesktop(QWidget* parent,DJGamePanel *panel)
			: DJGamePokerDesktop(parent,panel)
{
	c_UpgradePanel = (UpgradePanel *)panel;
	
	connect(this,SIGNAL(gameStarted()),this,SLOT(PlayerStarted()));
	PUpgradeRoom pugroom = (PUpgradeRoom)((c_UpgradePanel->gameRoom())->privateRoom());
...
#endif
	c_bShows[0]->setToolTip(tr("show club"));
	c_bShows[0]->setIcon( QIcon(":/BaseRes/image/desktop/toolbutton/c.png") );
...
#endif
	c_bShows[1]->setToolTip(tr("show diamond"));
	c_bShows[1]->setIcon( QIcon(":/BaseRes/image/desktop/toolbutton/d.png") );
...
#endif
    djDebug()<<"pugroom->chCards"<<pugroom->chCards;
...
void UpgradeDesktop::ModifyPlayerName()
{
	QString str[2];
	DJGameUser *puser;
	quint32 uid;
	...
	for(int i=1;i<=4;i++)
	{
		uid = c_UpgradePanel->userIdOfSeat(i);
		puser = c_UpgradePanel->gameUser(uid);
		if ( !puser )
			continue;
		if(i<3)
		{
	...
	if(c_UpgradePanel->selfSeatId()%2 == 0)
	{
...
void UpgradeDesktop::ModifyScoreNumber()
{
	c_ScoreNumber->setText(QString("%1").arg(c_shCurrentScore));
}
void UpgradeDesktop::ModifyShow(quint8 seat,quint8 chCard)
{
	...
	DJGameUser *puser;
	quint32 uid;
	uid = c_UpgradePanel->userIdOfSeat(seat);
	puser = c_UpgradePanel->gameUser(uid);
	if ( puser )
...
void UpgradeDesktop::ModifyMaster()
{
	...
	}else
	{
		DJGameUser *puser;
		quint32 uid;
		uid = c_UpgradePanel->userIdOfSeat(c_current.chMaster);
		puser = c_UpgradePanel->gameUser(uid);
		if ( puser )
...
void UpgradeDesktop::PreviousPressed()
{
...
void UpgradeDesktop::PreviousRelesed()
{
...
void UpgradeDesktop::SubtractPressed()
{
	RepaintAttachCards(c_current.chMaster,c_chSubstract,c_chSubstractPages);
}
void UpgradeDesktop::SubtractReleased()
{
	ClearSeatAttachCard(c_current.chMaster);
}

void UpgradeDesktop::GetPageInterval(quint8 chView,quint8 *pcards,quint8 chPages,int *dx,int *dy)
{
	(*dx) = 0;
	(*dy) = 0;
	if(chView != 1)
...
bool UpgradeDesktop::SameCardClass(quint8 chCard1,quint8 chCard2)
{
...
void UpgradeDesktop::sortCards( quint8 *pCards, quint8 len )
{
  	quint8 iTemp;
  	for(int i=1;i<len;i++)
  	{
...
int UpgradeDesktop::Compare2Card(quint8 chCard1,quint8 chCard2,PDJGamePokerImage pimage)
{
...
#if QT_VERSION >= 0x040200
void UpgradeDesktop::dealCard(int currentCardFrame )
{
	djDebug()<<"UpgradeDesktop::dealCard";
	...
	for(i=1;i<=4;i++)
	{
		...
		if( c_chPlayerCardPages[i] > 0 )
		{
			...
			//RepaintSeatCard(i);
			if(i==c_UpgradePanel->selfSeatId()) {
				RepaintSeatCard(i);
				c_UpgradePanel->playWave("out.wav");
				CheckShowStatus();
			}
		}
	}
}
void UpgradeDesktop::dealCardFinished()
{
	for(int i=1;i<=4;i++)
	{
		RepaintSeatCard(i);
...
#else
void UpgradeDesktop::HandleDrawTimeout()
{
	djDebug()<<"UpgradeDesktop::HandleDrawTimeout";
	...
	if(j ==  1)
		...
		c_drawTimer->start(300);
		*/
}
#endif
void UpgradeDesktop::ShowHeart()
{
	quint8 chPages = GetCardPages(c_UpgradePanel->selfSeatId(),DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_H,c_current.chLevel));
...
void UpgradeDesktop::ShowSpade()
{
	quint8 chPages = GetCardPages(c_UpgradePanel->selfSeatId(),DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_S,c_current.chLevel));
	if(chPages > 0 && (c_chLastShow == 0 || chPages >= DJGAME_POKER_PAGES(c_chLastShow)))
		ShowCard(((chPages-1)<<6)+DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_S,c_current.chLevel));
}
void UpgradeDesktop::ShowDiamond()
{
	quint8 chPages = GetCardPages(c_UpgradePanel->selfSeatId(),DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_D,c_current.chLevel));
	if(chPages > 0 && (c_chLastShow == 0 || chPages >= DJGAME_POKER_PAGES(c_chLastShow)))
		ShowCard(((chPages-1)<<6)+DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_D,c_current.chLevel));
}
void UpgradeDesktop::ShowClover()
{
	quint8 chPages = GetCardPages(c_UpgradePanel->selfSeatId(),DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_C,c_current.chLevel));
	if(chPages > 0 && (c_chLastShow == 0 || chPages >= DJGAME_POKER_PAGES(c_chLastShow)))
		ShowCard(((chPages-1)<<6)+DJGAME_POKER_MAKE_CODE(0,DJGAME_POKER_CLASS_C,c_current.chLevel));
}
void UpgradeDesktop::ShowNT()
{
	quint8 chPages = GetCardPages(c_UpgradePanel->selfSeatId(),DJGAME_POKER_SMALLKING);
	if(chPages > 1 && (c_chLastShow == 0 || chPages >= DJGAME_POKER_PAGES(c_chLastShow)))
		ShowCard(((chPages-1)<<6)+DJGAME_POKER_SMALLKING);
	else
	{
		chPages = GetCardPages(c_UpgradePanel->selfSeatId(),DJGAME_POKER_BIGKING);
		if(chPages > 1 && (c_chLastShow == 0 || chPages >= DJGAME_POKER_PAGES(c_chLastShow)))
			ShowCard(((chPages-1)<<6)+DJGAME_POKER_BIGKING);
	}
}
void UpgradeDesktop::NoShow()
{
}

void UpgradeDesktop::ShowCard(quint8 chCard)
{
	SendGameTrace(UPGRADE_GAMETRACE_SHOW,(char *)&chCard,1);
}
void UpgradeDesktop::ClickThrow()
{
	djDebug() << "UpgradeDesktop::ClickThrow";
	DJGamePokerDesktop::handleItemClicked(Qt::RightButton,QPoint(),0);
}
bool UpgradeDesktop::isTrump( const DJPoker& poker ) const
{
...
int UpgradeDesktop::suitOfPoker( const DJPoker& poker ) const
{
	if ( isTrump(poker) )
...
QList<DJPoker> UpgradeDesktop::pokersOfSuit( quint8 seat, int suit, QList<DJPoker>& leftPokers  ) const
{
...
DJPoker UpgradeDesktop::higherSerialPoker( const DJPoker& poker, bool isStrict ) const
{
	DJPoker::Point point	= poker.point();
	int	suit	= poker.suit();
	DJPoker trumpLevelPoker( c_chLastShow & 0x3f, isBridgeCard() );
...
DJPoker UpgradeDesktop::lowerSerialPoker( const DJPoker& poker, bool isStrict ) const
{
	DJPoker::Point point	= poker.point();
	int	suit	= poker.suit();
	DJPoker trumpLevelPoker( c_chLastShow & 0x3f, isBridgeCard() );
...
int UpgradeDesktop::findMatchingWidth( QList<DJPokerPattern>& patterns, int width )
{
...
QList<DJPokerPattern>::iterator UpgradeDesktop::findMatchingPattern( QList<DJPokerPattern>& patterns,
...
void UpgradeDesktop::CheckShowStatus()
{
	int i;
	if(GetTableStatus() != DJGAME_UPGRADE_TABLE_STATUS_WAIT_SHOW)
		c_lShow->hide();
	if(c_UpgradePanel->isLookingOn() || !IsWaittingForMe())
	{
		for(i=0;i<=5;i++)
            c_bShows[i]->setEnabled(false);
		return;
	}
	quint8 seat = c_UpgradePanel->selfSeatId();
	...
	}else
	{
		...
		/*
		if(DJGAME_POKER_ISBK(c_chLastShow))
			c_bShows[4]->setEnabled(false);
		else if(DJGAME_POKER_ISSK(c_chLastShow))
		{
			if( (pokerimg->chPage[DJGAME_POKER_BIGKING] >= 2 &&  pokerimg->chPage[DJGAME_POKER_BIGKING] >= chPages))
				c_bShows[4]->setEnabled(true);
			else
				c_bShows[4]->setEnabled(false);
		}else 
		{
			if( (pokerimg->chPage[DJGAME_POKER_BIGKING] >= 2 &&  pokerimg->chPage[DJGAME_POKER_BIGKING] >= chPages) ||
					(pokerimg->chPage[DJGAME_POKER_SMALLKING] >= 2 &&  pokerimg->chPage[DJGAME_POKER_SMALLKING] >= chPages))
				c_bShows[4]->setEnabled(true);
			else
				c_bShows[4]->setEnabled(false);
		}
...
UpgradeDesktop::~UpgradeDesktop()
{
}

void UpgradeDesktop::handleMatrixChanged(const QMatrix &m)
{
...
void UpgradeDesktop::PlayerStarted()
{
	//StaticInitDesktop();
...
void UpgradeDesktop::StaticInitDesktop()
{
...
void UpgradeDesktop::StaticGameTrace(GeneralGameTrace2Head *gameTrace)
{
	djDebug() << "UpgradeDesktop::StaticGameTrace";
	switch(gameTrace->chType)
	{
		...
		case UPGRADE_GAMETRACE_THROW : 
		{
			...
			if(c_bFirstPlayer == true)
			{
				...
				if(gameTrace->chSite != c_UpgradePanel->selfSeatId())
					c_bSelfThrow = false;
		...
		case UPGRADE2_GAMETRACE_CLEARDESKTOP : 
		{
			...
			DJGameRoom *room= c_UpgradePanel->gameRoom();
		...
		{
			djDebug() << "UPGRADE_GAMETRACE_MASTER" << "seat" << gameTrace->chSite;
			m_masterSeat	= gameTrace->chSite;
		...
		{
			djDebug() << "UPGRADE_GAMETRACE_SHOW" << "seat" << gameTrace->chSite;
		...
		{
			djDebug() << "UPGRADE2_GAMETRACE_HANDS" << "seat" << gameTrace->chSite;
			DrawOver(false);
		...
		{
			djDebug() << "UPGRADE3_GAMETRACE_SUBSTRACT" << "seat" << gameTrace->chSite;
			DrawOver(false);
			if(c_current.chMaster == 0)
			{
				c_current.chMaster = gameTrace->chSite;
...
void UpgradeDesktop::RepaintCurrentStatus()
{
	DJGameRoom *room= c_UpgradePanel->gameRoom();
	...
	quint8 seat;
	if(c_current.chMaster%2 == c_UpgradePanel->selfSeatId()%2)
	{
		seat = (c_UpgradePanel->selfSeatId()==1)?4:c_UpgradePanel->selfSeatId()-1;
	}else
		seat = c_UpgradePanel->selfSeatId();
...
bool UpgradeDesktop::IsThrowStep(PGeneralGameTrace2Head ptrace)
{
	if(ptrace->chType == UPGRADE_GAMETRACE_THROW)
		return true;
	return false;
}
void UpgradeDesktop::userArrived( quint32 userId, quint8 seatId )
{
...
void UpgradeDesktop::gameInfo( const unsigned char *buf )
{
	djDebug() << "UpgradeDesktop::gameInfo";
...
void UpgradeDesktop::gameWait( quint16 mask, quint8 status ,quint16 timeout)
{
	...
	switch(status)
	{
		...
		case DJGAME_UPGRADE_TABLE_STATUS_WAIT_OUT :
		{
			...
			if(IsWaittingForMe())
			{
				...
				m_throw->setEnabled( true );
#endif
				quint8 selfSeat	= c_UpgradePanel->selfSeatId();
				quint8 prevSeat	= prevSeatId( selfSeat );
				quint8 prevSeatPages	= c_ThrowCards[prevSeat][0][0];
				
				djDebug() << m_masterSeat << selfSeat << numberOfCards(selfSeat) << prevSeat << prevSeatPages ;
...
void UpgradeDesktop::DrawOver(bool bRepaint)
{
	djDebug()<<"UpgradeDesktop::DrawOver"<<bRepaint;
#if QT_VERSION < 0x040200	
	c_drawTimer->stop();
#endif	
	for(int i=1;i<=4;i++)
...
void UpgradeDesktop::gameTrace( const GeneralGameTrace2Head *gameTrace )
{
	DJGameDesktop::gameTrace(gameTrace);
	GeneralGameTrace2Head *ptrace = (PGeneralGameTrace2Head)gameTrace;
	djDebug() << "UpgradeDesktop::gameTrace seat" << gameTrace->chSite << "type" << gameTrace->chType;
...
void UpgradeDesktop::RecvThrowACL(bool bFailed)
{
...
void UpgradeDesktop::CreateThrowTrace(PGeneralGameTrace2Head ptrace)
{
	...
	if(GetTableStatus() == DJGAME_UPGRADE_TABLE_STATUS_WAIT_OUT)
	{
			...
			if(ptrace->chBufLen > 1)
			{
				for(int i=1;i<ptrace->chBufLen;i++)
				{
					...
					{//多于一种花色
						...
						DJMessageBox::information(10,c_UpgradePanel,tr("throw card failed"),tr("You must throw cards with the same suit"),QMessageBox::Ok);
						ptrace->chBufLen = 0;
						return;
					}
				}
		...
		}else
		{
			...
			{//跟牌张数不对
				//printf("pages error\n");
				djDebug() << "pages error";
				DJMessageBox::information(10,c_UpgradePanel,tr("throw card failed"),tr("You must throw %1 cards").arg(c_chFirstPages),QMessageBox::Ok);
				ptrace->chBufLen = 0;
				return;
			}
		}
	...
	}else if(GetTableStatus() == DJGAME_UPGRADE_TABLE_STATUS_WAIT_HANDS)
	{
		...
		{//扣牌张数不对
			...
			DJMessageBox::information(10,c_UpgradePanel,tr("discard cards failed"),tr("You must discard %1 cards").arg(pugroom->chHands),QMessageBox::Ok);
			ptrace->chBufLen = 0;
			return;
		}
...
void UpgradeDesktop::MoveItem(Q3CanvasItem* item,QPoint &ps,QPoint &pe)
{
	DJGameDesktop::MoveItem(item,ps,pe);
}
bool UpgradeDesktop::handleItemClicked(Qt::MouseButton btn,const QPoint& pos,Q3CanvasItem * item)
{
...
void UpgradeDesktop::RepaintView1Cards(quint8 seat,quint8 *cards,quint8 chPages)
{
...
/////////////////////////////////////////////////
UpgradePanel::UpgradePanel( DJGameController *gameController, DJGameRoom *room, const QList<quint32>& userIds, quint8 tableId, quint8 seatId, QWidget * parent, Qt::WFlags f)
...
{	
	...
	UpgradeDesktop* gameDesktop = new UpgradeDesktop(gameSplitter,this);
